...
//Retornando dados do Recibo
$idrecibo = 0;
if(isset($_GET['idrecibo']) && empty($_GET['idrecibo']) == false){
    $idrecibo = addslashes($_GET['idrecibo']);
}else{
    header("Location: meusrecibos.php");
}

//Consultando recibos inseridos pelo usuário atual
$sql = "SELECT * FROM  recibo WHERE idrecibo = '$idrecibo'";
$sql = $pdo->query($sql);
//Valores do Banco
if($sql->rowCount()>0){
